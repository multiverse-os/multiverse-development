...
use portal_util::{get_time, ClockType};
...
const DEFAULT_KERNEL_CMDLINE: &str = "panic=1 pci=on nomodules 8250.nr_uarts=0 \
                                      i8042.noaux i8042.nomux i8042.nopnp i8042.dumbkbd";
...
/// Portal was shut down after intercepting a restricted system call.
pub const FC_EXIT_CODE_BAD_SYSCALL: u8 = 148;
/// Portal was shut down after intercepting `SIGBUS`.
pub const FC_EXIT_CODE_SIGBUS: u8 = 149;
/// Portal was shut down after intercepting `SIGSEGV`.
pub const FC_EXIT_CODE_SIGSEGV: u8 = 150;
/// Invalid json passed to the Portal process for configuring microvm.
...
/// input, but can result from bad configuration of the host (for example if Portal doesn't
...
pub enum Error {
    ...
    /// Cannot open /dev/kvm. Either the host does not have KVM or Portal does not have
...
impl EpollContext {
    ...
    // TODO: REQUIRE A REGISTERED KEY FROM THE OWNER TO INPUT STDIN EVENTS!
...
/// Used for configuring a vmm from one single json passed to the Portal process.
#[derive(Deserialize)]
...
/// Contains the state and associated methods required for the Portal VMM.
pub struct Vmm {
    kvm: KvmContext,
...
impl Vmm {
    ...
    fn start_vcpus(&mut self, mut vcpus: Vec<Vcpu>) -> std::result::Result<(), StartMicrovmError> {
        ...
        for cpu_id in (0..vcpu_count).rev() {
            ...
            self.vcpus_handles.push(
                thread::Builder::new()
                    .name(format!("portal_vcpu{}", cpu_id))
                    .spawn(move || {
    ...
    /// Waits for all vCPUs to exit and terminates the Portal process.
    fn stop(&mut self, exit_code: i32) {
        ...
        // Exit from Portal using the provided exit code. Safe because we're terminating
        // the process anyway.
        unsafe {
    ...
    fn init_logger(
        ...
        let portal_version;
        {
            ...
            portal_version = guard.vmm_version.clone();
        }
        ...
        LOGGER
            .init(
                &AppInfo::new("Portal", &portal_version),
