...
/// TODO: Lets switch name from tun0 to ens0 
...
fn dup2(old_fd: libc::c_int, new_fd: libc::c_int) -> Result<()> {
    ...
    SyscallReturnCode(unsafe { libc::dup2(old_fd, new_fd) }).into_empty_result().map_err(Error::Dup2)
}
...
fn get_value<'a>(args: &'a ArgMatches, arg_name: &'static str) -> Result<&'a str> {
    args.value_of(arg_name).ok_or_else(|| Error::MissingArgument(&arg_name))
}
...
impl Env {
    pub fn new(args: ArgMatches, start_time_us: u64, start_time_cpu_us: u64) -> Result<Self> {
        ...
        chroot_dir.push(
            exec_file_path.file_name().ok_or_else(|| Error::FileName(exec_file_path.clone()))?,
        );
        chroot_dir.push(id);
        ...
        let extra_args = args.values_of("extra-args").into_iter().flatten().map(String::from).collect();
    ...
    pub fn run(mut self, socket_file_name: &str) -> Result<()> {
        ...
        let dev_null = if self.daemonize {
            ...
            Some(
                SyscallReturnCode(unsafe {
                    libc::open(DEV_NULL_WITH_NUL.as_ptr() as *const libc::c_char, libc::O_RDWR)
                })
                .into_result()
        ...
        // Change ownership of the jail root to Portal's UID and GID. This is necessary
        // so Portal can create the unix domain socket in its own jail.
