...
const SOCKET_FILE_NAME: &str = "portal-api.socket";
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            Exec(ref err) => write!(f, "Failed to exec into Portal: {}", err),
...
pub fn run(args: ArgMatches, start_time_us: u64, start_time_cpu_us: u64) -> Result<()> {
    ...
    // passed on to Portal post exec, and used via knowing their values in advance.

    // TODO: can a malicious guest that takes over portal use its access to the KVM fd to
    ...
    fs::create_dir_all(env.chroot_dir()).map_err(|e| Error::CreateDir(env.chroot_dir().to_owned(), e))?;
...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert_eq!(
            ...
            format!("Failed to exec into Portal: {}", err2_str)
        );
        assert_eq!(
