...
// How is this helpful? When creating cgroup folders for the jailer Portal instance, the jailer
// will create a hierarchy that looks like <cgroup_base>/portal/<id>. Depending on each
...
// usually contain a single line with some configuration value(s). When the "portal" and <id>
...
// if <cgroup_base>/portal/some_file is empty, then we cannot have a non-empty file with
// at <cgroup_base>/portal/<id>/some_file. The inherit_from_parent function (which is based
...
// inherit_from_parent_aux(/A/portal/id1, file, true), and hits case number 3) from the list
// above, thus recursively calling inherit_from_parent_aux(/A/portal, file, false). It's
// entirely possible there was another process in the exact same situations, and that process
// gets to write something to /A/portal/file first. In this case, the recursive call made by
// the first process to inherit_from_parent_aux(/A/portal, file, false) may fail when writing
// to /A/portal/file, but we can still continue, because step 4) only cares about the file
