...
impl ApiServer {
    ...
    pub fn bind_and_run(
        ...
        if let Some(start_time) = start_time_us {
            let delta_us = (portal_util::get_time(portal_util::ClockType::Monotonic) / 1000) - start_time;
            METRICS
                .api_server
        ...
        if let Some(cpu_start_time) = start_time_cpu_us {
            let delta_us =
                portal_util::get_time(portal_util::ClockType::ProcessCpu) / 1000 - cpu_start_time;
            METRICS
                .api_server
