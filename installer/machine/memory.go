...
func ParseStructureTableAddressEFI(f *os.File) (address int64, length int, err error) {
...
func ParseStructureTableAddress(f *os.File) (address int64, length int, err error) {
...
func ParseStructureTable() ([]byte, error) {
	...
	address, length, err := ParseStructureTableAddressEFI(f)
	if err != nil {
		if address, length, err = ParseStructureTableAddress(f); err != nil {
			return nil, err
		}
	}
...
// TODO: Really really hate these impelemtnations that use GOTO; I don't ever understand why this is preferable over a standard loop.
func (self *Machine) ParseMemory() {
	mem, err := ParseStructureTable()
	if err != nil {
		if targetKB := slurpFile("/sys/devices/system/xen_memory/xen_memory0/target_kb"); len(targetKB) != 0 {
			self.Memory.Type = "DRAM"
			size, _ := strconv.ParseUint(targetKB, 10, 64)
			self.Memory.Size = uint(size) / 1024
		}
		return
	}
	defer syscall.Munmap(mem)
...
loop:
	for p := 0; p < len(mem)-1; {
		...
		case 4:
			if self.CPU.Speed == 0 {
				self.CPU.Speed = uint(word(mem, p+0x16))
			}
		case 17:
			...
			if len(self.Memory.Type) == 0 {
				...
				if index := int(mem[p+0x12]); index >= 1 && index <= len(memTypes) {
					self.Memory.Type = memTypes[index-1]
				}
			}

			if self.Memory.Speed == 0 && recLen >= 0x17 {
				if speed := uint(word(mem, p+0x15)); speed != 0 {
					self.Memory.Speed = speed
				}
			}
		case 19:
	...
	if self.Memory.Size == 0 && memSizeAlt > 0 {
		self.Memory.Type = "DRAM"
		self.Memory.Size = memSizeAlt
	}
}
