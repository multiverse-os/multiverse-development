...
func PortType(supp uint32) (port string) {
...
func ParseMaxSpeed(supp uint32) (speed uint) {
	// Fancy, right?
	switch {
...
func ParseSupported(name string) uint32 {
...
func (self *Machine) ParseNetwork()  {
	...
	self.Network = make([]NetworkDevice, 0)
	for _, link := range devices {
		...
		supportedLinkName := ParseSupported(link.Name())

		device := NetworkDevice{
			Name:       link.Name(),
			MACAddress: slurpFile(path.Join(fullpath, "address")),
			Port:       PortType(supportedLinkName),
			Speed:      ParseMaxSpeed(supportedLinkName),
		}
		...
		self.Network = append(self.Network, device)
	}
}
