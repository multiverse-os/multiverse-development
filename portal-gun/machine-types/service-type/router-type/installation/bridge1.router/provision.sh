...
<<<<<<< HEAD
## TODO:
# Ideally we want to move all unique values in the script to variables
# so we can begin merging provision scripts and eventually when we move to
# replace these with Go we will be able to easily determine what values
# should go into YAML config files so that we can have a single provisioning
# script that loads unique data for a given portal from a YAML config. 
## TODO:
# Especially with having multiple Galaxy routers at a time we need 
# to copy and edit, its not as simple as universe, which is also not
## TODO:
# Just wait until Go conversion but need to track which galaxy router this
# is for setups with more than one. Some current installs have up to three. 
# exaclty setup.
#
# most will only have 1 but
# one of our develoeprs has a 3 router implementatation
# and more than one will likely be common
...
router_type="bridge1"
>>>>>>> 22e4ae48717a034840c3294a04867e96c878dd4a
router_instance="0" 
...
# Create necessary paths for local base, arguably should all
# be just copied directly but this method allows for future
# router specific p9 shares for simplifed editing.
mkdir -p $local_path

## Alpine linux OS base provisioning directories ##############################
mkdir -p $local_base_files
mkdir -p $local_base_files/etc/
mkdir -p $local_base_files/etc/
mkdir -p $local_base_files/etc/network
mkdir -p $local_base_files/etc/sysctl.d
mkdir -p $local_base_files/etc/local.d/
mkdir -p $local_base_files/etc/periodic/
mkdir -p $local_base_files/etc/periodic/15min
mkdir -p $local_base_files/etc/periodic/hourly
mkdir -p $local_base_files/etc/periodic/daily
mkdir -p $local_base_files/etc/periodic/weekly
mkdir -p $local_base_files/etc/periodic/monthly

if [ $router_type = "galaxy" ]; then
=======
...
<<<<<<< HEAD
mkdir -p $local_base_files/etc/openvpn
mkdir -p $local_base_files/etc/shorewall
mkdir -p $local_base_files/etc/dhcp
...
###############################################################################
## TODO:
#    Ideally the below argument checking functionality should be moved into
...
#    of the script, it will be broken into sections similar to the
#    `setup-alpine`
#
#    script which is made up of a lot of sub scritps that can be ran
#    individually or run together in order when running the primary
#    `setup-alpine`. 
#
###############################################################################
...
if [ -z "$1" ]; then
	...
	elif [ "$1" = "-c" -o "$1" = "--copy" -o "$1" = "-s" -o "$1" = "--std" ]; then
		dev_mode=0
...
# TODO: A flag for cleanup to be ran at the very end before snapshotting, and
# preferably making the snapshotting and backup up apart of the provisioning
# process.
...
#if [ $dev_mode -eq 0 ]; then
#	echo -e $subheader"[Standard Mode]$reset$text Copying configuration files from p9 shared storage..."$reset
	cp $base_files/etc/shorewall/hosts          $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/interfaces     $local_base_files/etc/shorewall/  2>/dev/null   
	cp $base_files/etc/shorewall/masq           $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/policy         $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/rules          $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/shorewall.conf $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/snat           $local_base_files/etc/shorewall/  2>/dev/null
	cp $base_files/etc/shorewall/zones          $local_base_files/etc/shorewall/  2>/dev/null
#else
#	echo -e $subheader"[Development Mode]$reset$text Creating symbolic links from p9 shared storage..."$reset
	ln -s $local_base_files/etc/shorewall/hosts          /etc/shorewall/  2>/dev/null
	ln -s $local_base_files/etc/shorewall/interfaces     /etc/shorewall/  2>/dev/null
	ln -s $local_base_files/etc/shorewall/masq           /etc/shorewall/  2>/dev/null  
	ln -s $local_base_files/etc/shorewall/policy         /etc/shorewall/  2>/dev/null 
	ln -s $local_base_files/etc/shorewall/rules          /etc/shorewall/  2>/dev/null 
	ln -s $local_base_files/etc/shorewall/shorewall.conf /etc/shorewall/  2>/dev/null 
	ln -s $local_base_files/etc/shorewall/snat           /etc/shorewall/  2>/dev/null 
	ln -s $local_base_files/etc/shorewall/zones          /etc/shorewall/  2>/dev/null 
#fi
=======
	cp $base_files/etc/dhcp/dhcpd.conf /etc/dhcp/    2>/dev/null
#===============================================================================
...
#if [ $dev_mode -eq 0 ]; then
#	echo -e $subheader"[Standard Mode]$reset$text Copying configuration files from p9 shared storage..."$reset
	cp $base_files/etc/network/interfaces $local_base_files/etc/network/    2>/dev/null
#else
#	echo -e $subheader"[Development Mode]$reset$text Creating symbolic links from p9 shared storage..."$reset
	ln -s $local_base_files/etc/network/interfaces /etc/network/interfaces 2>/dev/null
#fi
#===============================================================================
...
if [ $router_type = "galaxy" ]; then
	...
	cp $base_files/etc/openvpn/up.sh $local_base_files/etc/openvpn/    2>/dev/null
	cp $base_files/etc/openvpn/down.sh $local_base_files/etc/openvpn/    2>/dev/null
=======
...
<<<<<<< HEAD
	...
	cp $base_files/etc/sysctl.d/05-multiverse.conf $local_base_files/etc/sysctl.d/    2>/dev/null

#else
	#echo -e $text"[Development Mode] Creating symbolic links from p9 shared storage..."$reset
	ln -s $local_base_files/etc/sysctl.d/05-multiverse.conf /etc/sysctl.d/ 2>/dev/null
#fi
=======
...
#if [ $dev_mode == 0 ]; then
	#echo -e $subheader"[Standard Mode]$reset$text Copying configuration files from p9 shared storage..."$reset
	cp $base_files/etc/hosts $local_base_files/etc/ 2>/dev/null
	cp $base_files/etc/issue $local_base_files/etc/ 2>/dev/null
	cp $base_files/etc/motd  $local_base_files/etc/ 2>/dev/null
#else
	#echo -e $subheader"[Development Mode]$reset$text Creating symbolic links from p9 shared storage..."$reset
	ln -s $local_base_files/etc/hosts /etc/ 2>/dev/null
	ln -s $local_base_files/etc/issue /etc/ 2>/dev/null
	ln -s $local_base_files/etc/motd  /etc/ 2>/dev/null
#fi
=======
